  ...
  outputs = { self, nixpkgs, nixCats, ... }@inputs: let
    ...
    dependencyOverlays = /* (import ./overlays inputs) ++ */ [
      ...
      (final: prev: {
        strudel-nvim = prev.pkgs.buildNpmPackage {
          ...
          # We don't need a build phase for the vim plugin itself, 
          # just for the node_modules.
          dontBuild = true;
          ...
          # Copy the source + the newly built node_modules to the output
          installPhase = ''
            mkdir -p $out
            cp -r . $out
            # buildNpmPackage handles node_modules, but we ensure structure here
            # depending on how buildNpmPackage behaves, it might be automatic,
            # but this ensures the plugin root is correct.
            # --- ADD THIS LINE ---
            # Ensure the node scripts are executable
            find $out/node_modules -type f -name "*.js" -exec chmod +x {} \;
          '';
        };
      })
    ];
    ...
    categoryDefinitions = { pkgs, settings, categories, extra, name, mkPlugin, ... }@packageDef: {
      ...
      environmentVariables = {
        ...
        customPlugins = {
           # Puppeteer reads this variable to find the browser
